...
#TODO： 1、删除pad_sequence操作，删除shape相关操作
class RNN(nn.Module):
...
class SubPoser(nn.Module):
    ...
    def forward(self, x, v_init, p_init):
        if self.extra_dim != 0:
            x_v = x[:, :-self.extra_dim]   # remove glb information when predict local part velocity
...
class Poser_convert(nn.Module):
    def __init__(self):
        super(Poser_convert, self).__init__()
    ...
    def forward(self, x, v_init, p_init):
        ...
        s_glb = self.glb(x.flatten(1))
        for i in range(len(self.posers)):
            sensorq = x[:, self.indices[i]['sensor_indices']].flatten(1)                    # parts = [x[:, 0], x[:, 4], x[:, 5]]
                                                                                          # sensor = torch.cat(parts, dim=1).flatten(1)
            sensor_indices = self.indices[i]['sensor_indices']
            slices = [x[:, idx] for idx in sensor_indices]
            sensor = (torch.cat(slices, dim=1).flatten(1))
            si  = torch.cat((sensor, s_glb), dim=-1)
            #   vi = v_init[:, self.indices[i]['v_indices']].flatten(1)
            vi_indices = self.indices[i]['v_indices']
            slices = [v_init[:, idx] for idx in vi_indices]
            vi = (torch.cat(slices, dim=1).flatten(1))
            #   pi = p_init[:, self.indices[i]['p_indices']].flatten(1)
            pi_indices = self.indices[i]['p_indices']
            slices = [p_init[:, idx] for idx in pi_indices]
            pi = (torch.cat(slices, dim=1).flatten(1))
            v, p = self.posers[i](si, vi, pi)
            v_out.append(v)
            p_out.append(p)

        v_out = torch.cat(v_out, dim=1)  #v_out:{list:1,[sequence,12] } p:{list:1,[sequence,12] }
        p_out = torch.cat(p_out, dim=1)  #p_oout
