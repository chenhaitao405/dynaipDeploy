...
while len(received_data) != 216:
    time.sleep(0.1)
body_pose, global_orient = get_current_data()

body_pose = body_model.inverse_kinematics_R(body_pose).view(body_pose.shape[0], 24, 3, 3)
body_pose = art.math.rotation_matrix_to_axis_angle(body_pose).unsqueeze(dim=0)
body_pose = body_pose[3:72].reshape(1, 23, -1)

output = model(betas=betas.float(), 
               body_pose=body_pose.float(),   # 24,3
               global_orient=global_orient.float(), 
               return_verts=True)

initial_vertices = output.vertices[0].detach().cpu().numpy()
...
def animation_callback(vis):
    ...
    if body_pose is None:
        ...
        return True
    output = model(betas=betas, body_pose=body_pose, global_orient=global_orient, return_verts=True)
