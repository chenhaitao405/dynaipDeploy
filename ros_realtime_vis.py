...
#     if len(received_data) != 72:
#         return None, None  # 数据不完整
    
#     vis_data = np.array(received_data)[3:72].reshape(1, 23, -1)
#     body_pose = torch.from_numpy(vis_data).to(torch.float32)
#     # global_data = np.zeros((1, 1, 3))  # 固定腰部
#     global_data = np.array(received_data)[:3].reshape(1, 1, -1)  # 不固定腰部
